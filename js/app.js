...
async function downloadFile() {
    await sendTelegram('O usuário iniciou o download do APK');
    
    try {
        // Tenta obter a listagem de arquivos da pasta apk/
        // Isso funciona se o servidor tiver a listagem de diretórios (directory listing) habilitada
        ...
        // Procura pelo primeiro link que termina em .apk
        const apkLink = links.find(link => {
            const href = link.getAttribute('href');
            return href && href.toLowerCase().endsWith('.apk');
        });

        if (apkLink) {
            const apkName = apkLink.getAttribute('href');
            const downloadUrl = `apk/${apkName}`;
        ...
        console.warn('Não foi possível listar a pasta apk/ automaticamente:', error);
    }

    // Fallback: Se a listagem falhar ou não encontrar nada, tenta baixar o arquivo 'app.apk'
    // ou o link do Telegram como estava antes, mas agora priorizando a pasta local
    const fallbackLink = document.createElement('a');
    fallbackLink.href = 'apk/app.apk';
    fallbackLink.download = 'app.apk';
...
function addToWishlist() {
    ...
    toast.textContent = '✓ Adicionado à lista de desejos';
    toast.style.cssText = `
        position: fixed;
        bottom: 100px;
        left: 50%;
